...
progressr::with_progress({
  ...
  pbp_games <- purrr::map_dfr(sort(years_vec, decreasing = TRUE), function(y){
    ...
    pbp_g <- data.frame()
    pbp_list <- list.files(path = glue::glue('{path_to_raw}/{y}/'))
    print(glue::glue('{path_to_raw}/{y}/'))
    pbp_g <- furrr::future_map_dfr(pbp_list, function(x){
      pbp <- jsonlite::fromJSON(glue::glue('{path_to_raw}/{y}/{x}'))$plays
    ...
    ifelse(!dir.exists(file.path(glue::glue("{path_to_pbp}/csv"))), dir.create(file.path(glue::glue("{path_to_pbp}/csv"))), FALSE)
    write.csv(pbp_g, file=gzfile(glue::glue("{path_to_pbp}/csv/play_by_play_{y}.csv.gz")), row.names = FALSE)
    ifelse(!dir.exists(file.path(glue::glue("{path_to_pbp}/rds"))), dir.create(file.path(glue::glue("{path_to_pbp}/rds"))), FALSE)
    ...
    write.csv(dplyr::distinct(sched) %>% dplyr::arrange(desc(.data$date)),glue::glue('cfb/schedules/csv/cfb_schedule_{y}.csv'), row.names=FALSE)
...
write.csv(sched_g %>% dplyr::arrange(desc(.data$date)), 'cfb_schedule_2002_2021.csv', row.names = FALSE)

...
progressr::with_progress({
  ...
  pbp_games <- purrr::map_dfr(sort(years_vec, decreasing = TRUE), function(y) {
    ...
    pbp_g <- data.frame()
    pbp_list <- list.files(path = glue::glue("{path_to_raw}/"))
    print(glue::glue("{path_to_raw}/"))
    pbp_g <- furrr::future_map_dfr(pbp_list, function(x) {
      pbp <- jsonlite::fromJSON(glue::glue("{path_to_raw}/{x}"))$plays
      pbp$game_id <- as.integer(gsub(".json", "", x))
      return(pbp)
    })
    if (nrow(pbp_g) > 0) {
      pbp_g <- pbp_g %>%
    ...
    ifelse(!dir.exists(file.path(glue::glue("{path_to_pbp}"))), dir.create(file.path("cfb/pbp")), FALSE)
    ifelse(!dir.exists(file.path(glue::glue("{path_to_pbp}/rds"))),
           dir.create(file.path(glue::glue("{path_to_pbp}/rds"))),
           FALSE)
    saveRDS(pbp_g, glue::glue("{path_to_pbp}/rds/play_by_play_{y}.rds"))
    ifelse(!dir.exists(file.path("{path_to_pbp}/parquet")), dir.create(file.path("{path_to_pbp}/parquet")), FALSE)
    arrow::write_parquet(pbp_g, glue::glue("{path_to_pbp}/parquet/play_by_play_{y}.parquet"))
    sched <- read.csv(glue::glue("cfb/schedules/csv/cfb_schedule_{y}.csv"))
    sched <- sched %>%
      dplyr::mutate(
        status.displayClock = as.character(.data$status.displayClock),
        PBP = ifelse(.data$game_id %in% unique(pbp_g$game_id), TRUE, FALSE)
      )
    write.csv(dplyr::distinct(sched) %>% dplyr::arrange(desc(.data$date)),
              glue::glue("cfb/schedules/csv/cfb_schedule_{y}.csv"), row.names = FALSE)
...
all_games <- purrr::map(years_vec, function(y) {
  pbp_g <- pbp_games %>%
    ...
    dplyr::distinct()
    ifelse(!dir.exists(file.path(glue::glue("{path_to_pbp}"))),
           dir.create(file.path(glue::glue("{path_to_pbp}"))),
           FALSE)
    ifelse(!dir.exists(file.path(glue::glue("{path_to_pbp}/rds"))),
           dir.create(file.path(glue::glue("{path_to_pbp}/rds"))),
           FALSE)
    saveRDS(pbp_g, glue::glue("{path_to_pbp}/rds/play_by_play_{y}.rds"))
    ifelse(!dir.exists(file.path("{path_to_pbp}/parquet")),
           dir.create(file.path("{path_to_pbp}/parquet")),
           FALSE)
    arrow::write_parquet(pbp_g, glue::glue("{path_to_pbp}/parquet/play_by_play_{y}.parquet"))
})
sched_list <- list.files(path = glue::glue("cfb/schedules/csv/"))
sched_g <-  purrr::map_dfr(sched_list, function(x) {
  sched <- read.csv(glue::glue("cfb/schedules/csv/{x}")) %>%
    dplyr::mutate(
      status.displayClock = as.character(.data$status.displayClock)
    )
  return(sched)
...
write.csv(sched_g %>% dplyr::arrange(desc(.data$date)), "cfb_schedule_master.csv", row.names = FALSE)
write.csv(sched_g %>% dplyr::filter(.data$PBP == TRUE) %>% dplyr::arrange(desc(.data$date)), "cfb/cfb_games_in_data_repo.csv", row.names = FALSE)

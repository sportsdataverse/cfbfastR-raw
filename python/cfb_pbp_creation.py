...
path_to_pbp = 'cfb/pbp'
path_to_team_box = 'cfb/team_box'
path_to_player_box = 'cfb/player_box'
def cfb_team_box_score_extract(js, result):
...
def cfb_player_box_score_extract(js, result):
...
def main():
    ...
    for year in years_arr:
        ...
        season_schedule = pd.read_parquet(f"{path_to_schedules}/parquet/cfb_schedule_{year}.parquet", engine='auto', columns=None)
        ...
        for index, js in enumerate(json_files):
            try:
                # print("Processing game {} of {} (game_id: {})...".format(index+1, len(json_files), js))
                processed_data = sdv.cfb.CFBPlayProcess(gameId = js, path_to_json = path_to_final_json)
                # processed_data = sdv.cfb.CFBPlayProcess( gameId = 242620142, path_to_json = 'pbp_json_raw/')
                result = processed_data.cfb_pbp_disk()
                plays = pd.DataFrame(result.get("plays",[]))
                plays = plays.fillna(value=np.nan)
                team_box_score = cfb_team_box_score_extract(js, result)

                player_box_score = cfb_player_box_score_extract(js, result)
        ...
        if path_to_schedules is not None:
            ...
            pyreadr.write_rds(f"{path_to_pbp}/rds/play_by_play_{year}.rds", pbp_season)
            ...
            pyreadr.write_rds(f"{path_to_team_box}/rds/team_box_{year}.rds", team_box_season)
            ...
            pyreadr.write_rds(f"{path_to_player_box}/rds/player_box_{year}.rds", player_box_season)
            ...
            season_schedule.to_csv(f"{path_to_schedules}/csv/cfb_schedule_{year}.csv", index = False)
            season_schedule.to_parquet(f"{path_to_schedules}/parquet/cfb_schedule_{year}.parquet", index = False)
            pyreadr.write_rds(f"{path_to_schedules}/rds/cfb_schedule_{year}.rds", season_schedule)
            fp = f"{path_to_schedules}/json/cfb_schedule_{year}.json"
            with open(fp,'w') as f:

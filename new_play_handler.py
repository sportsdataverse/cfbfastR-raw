...
class PlayProcess(object):
    ...
    def cfb_pbp(self):
        ...
        for k in ['scoringPlays', 'standings', 'videos', 'pickcenter', 'broadcasts',
            'againstTheSpread', 'odds', 'predictor', 'winprobability','espnWP',
            'gameInfo', 'season']:
            pbp_txt[k]=self.key_check(obj=summary, key = k)
    ...
    def setup_penalty_data(self, play_df):
        ...
        play_df["penalty_text"] = None

        play_df.loc[(play_df.penalty_flag == True),'penalty_text'] = play_df.text.str.extract(r"Penalty(.+)", flags=re.IGNORECASE)[0]
        play_df["yds_penalty"] = None
        play_df.loc[(play_df.penalty_flag == True),'yds_penalty'] = play_df.penalty_text.str.extract("(.{0,3})yards|yds|yd to the ", flags=re.IGNORECASE)[0]
        play_df['yds_penalty'] = play_df['yds_penalty'].str.replace( " yards to the | yds to the | yd to the ", "", regex = True)
        play_df[(play_df.penalty_flag == True) &
            (play_df.text.str.contains(r"ards\)", case=False, regex=True)) &
            (play_df.yds_penalty.isna()),'yds_penalty'] = play_df.text.str.extract(r"(.{0,4})yards\)|Yards\)|yds\)|Yds\)",flags=re.IGNORECASE)[0]
    ...
    def add_rush_pass_flags(self, play_df):
        ...
        play_df.loc[
            ...
            (play_df["type.text"].isin(["Safety","Fumble Recovery (Opponent)","Fumble Recovery (Opponent) Touchdown", "Fumble Recovery (Own)", "Fumble Recovery (Own) Touchdown", "Fumble Return Touchdown"]) & play_df["text"].str.contains("run for"))), 'rush'] = True
    ...
    #--------------------------------------------------
        ...
        play_df.loc[
            play_df['type.text'].isin(["Kickoff Touchdown"]),'type.text'] = "Kickoff Team Fumble Recovery Touchdown"
        play_df.loc[
            (play_df['type.text'].isin(["Fumble Return Touchdown"])) &
            ((play_df["pass"] == True) | (play_df["rush"] == True)), 'type.text'] = "Fumble Recovery (Opponent) Touchdown"

        #--- Safeties (kickoff, punt, penalty) ----
        play_df.loc[(play_df['type.text'].isin(["Pass Reception", "Rush", "Rushing Touchdown"]) &
             ((play_df["pass"] == True) | (play_df["rush"] == True)) &
             (play_df["safety"] == True)), 'type.text'] = "Safety"

        play_df.loc[(play_df.kickoff_safety == True),'type.text'] = "Kickoff (Safety)"

        play_df.loc[(play_df.punt_safety == True), 'type.text'] = "Punt (Safety)"

        play_df.loc[(play_df.penalty_safety == True), 'type.text'] = "Penalty (Safety)"
        play_df.loc[
            (play_df['type.text'] == 'Extra Point Good') &
            (play_df["text"].str.contains("Two-Point", case=False, flags=0, na=False, regex=True)),'type.text'] = "Two-Point Conversion Good"

        play_df.loc[
            (play_df['type.text'] == 'Extra Point Missed') &
            (play_df["text"].str.contains("Two-Point", case=False, flags=0, na=False, regex=True)), 'type.text'] = "Two-Point Conversion Missed"
    ...
    def add_play_category_flags(self, play_df):
        ...
        play_df['yds_punt_gained'] = None
        play_df.loc[play_df.punt == True, 'yds_punt_gained'] = play_df["statYardage"]
